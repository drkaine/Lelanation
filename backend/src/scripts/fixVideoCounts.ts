...
async function fixVideoCounts() {
  ...
  try {
    ...
    for (const channel of storage.channels) {
      const videosForChannel = storage.videos.filter(v => v.snippet.channelId === channel.channelId);
      ...
      const totalVideosKnown = storage.metadata.totalVideosKnown[channel.channelId] || 0;
    ...
    for (const channel of storage.channels) {
      const totalVideosKnown = storage.metadata.totalVideosKnown[channel.channelId] || 0;
      const completionRate = Math.round((channel.videoCount / totalVideosKnown) * 100);
      console.log(`   - ${channel.channelName}: ${channel.videoCount}/${totalVideosKnown} (${completionRate}%)`);
    }
