...
async function testBothChannels() {
  ...
  try {
    ...
    for (const channel of channels) {
      ...
      console.log(
        `      - Videos: ${videos.length}/${stats.videoCount} (${completionRate}%)`,
      );
      console.log(
    ...
    for (const channel of channels) {
      ...
      console.log(
        `   - ${channel.channelName}: ${needsUpdate ? "🔄 NEEDS UPDATE" : "✅ UP TO DATE"}`,
      );
    }
    ...
    const overallCompletion = Math.round(
      (totalVideos / totalVideosOnYouTube) * 100,
    );
    ...
    console.log(
      `   - All channels complete: ${channels.every((c) => c.isComplete) ? "✅ YES" : "❌ NO"}`,
    );
